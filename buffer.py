...
class ReplayBuffer:
    def __init__(self, max_len,block_size, state_dim, action_dim, reward_dim, if_on_policy, if_per, if_gpu):
        ...
        if self.if_gpu:
            ...
            self.buf_state_past = torch.empty((max_len,block_size, state_dim), dtype=torch.float32, device=self.device)
            ...
            self.buf_state_past = np.empty((max_len,block_size, state_dim), dtype=np.float32)
    ...
    def append_buffer(self, state,state_past, action, reward, gamma):  # CPU array to CPU array
        if self.if_gpu:
            state = torch.as_tensor(state, dtype=torch.float32, device=self.device)
            state_past = torch.as_tensor(state_past, dtype=torch.float32, device=self.device)
            action = torch.as_tensor(action, dtype=torch.float32, device=self.device)
            reward = torch.as_tensor(reward, dtype=torch.float32, device=self.device)
            gamma = torch.as_tensor(gamma, dtype=torch.float32, device=self.device)
        self.buf_state[self.next_idx] = state
        self.buf_state_past[self.next_idx] = state_past
    ...
    def extend_buffer(self, state, state_past, action, reward, gamma):  # CPU array to CPU array
        if self.if_gpu:
            state = torch.as_tensor(state, dtype=torch.float32, device=self.device)
            state_past = torch.as_tensor(state_past, dtype=torch.float32, device=self.device)
            action = torch.as_tensor(action, dtype=torch.float32, device=self.device)
        ...
        if next_idx > self.max_len:
            if next_idx > self.max_len:
                ...
                self.buf_state_past[self.next_idx:self.max_len] = state_past[:self.max_len - self.next_idx]
            ...
            self.buf_state_past[0:next_idx] = state_past[-next_idx:]
            ...
            self.buf_state_past[self.next_idx:next_idx] = state_past
    ...
    def sample_all(self) -> tuple:
        ...
        return (torch.as_tensor(self.buf_reward[:self.now_len], device=self.device),
                ...
                torch.as_tensor(self.buf_state_past[:self.now_len], device=self.device))
...
class ReplayBufferMP:
    def __init__(self, max_len,block_size, state_dim, action_dim, reward_dim, if_on_policy, if_per, rollout_num):
        ...
        if self.if_on_policy:
            if_gpu = False
            if_per = False
        self.buffers = [ReplayBuffer(_max_len,block_size, state_dim, action_dim, reward_dim, if_on_policy, if_per, if_gpu=if_gpu)
                        for _ in range(rollout_num)]

    def extend_buffer(self, state,state_past, action, reward, gamma, i):
        self.buffers[i].extend_buffer(state,state_past, action, reward, gamma, )
